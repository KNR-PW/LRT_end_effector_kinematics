...
TEST(NewtonRaphsonTestAD, threeDofGradient)
{
  ...
  multi_end_effector_kinematics::NewtonRaphsonSolverAD solver(pinocchioInterface, modelInternalSettings, solverSettings);
...
TEST(NewtonRaphsonTestAD, SixDofGradient)
{
  std::string urdfPathName = package_path::getPath();
